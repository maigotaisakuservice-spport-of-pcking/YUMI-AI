...
​Version: 7.0 (Ultimate All-in-One: 1T-MoE, Studio-OS & Batch-Evolution)

YUMI AIは、既存のクラウドAPIや外部の計算サーバーに依存しない、完全自律型のサーバーレスAI・マルチモーダルOSです。

## 1. プロジェクト基本理念と究極目標
YUMI AIは、総計 **1兆（1T）パラメータ** の知識量をブラウザ（WebGPU）上で単独稼働させることを目的としています。これを実現するため「分離型MoE（10GB × 100 Experts）」アーキテクチャを採用し、API制限と運用コストをゼロにするため、GASとGoogleスプレッドシートを利用した「週次バッチ記憶統合（睡眠・記憶定着システム）」を搭載した、真のデジタル生命体です。

## 2. コア・アーキテクチャ (The 1T Decoupled MoE)

### 2.1 1兆パラメータの分離構造 (1T-MoE)
- **司令塔 (Router):** 約1GB。ブラウザに常駐。対話の受付、モード切り替え、対象エキスパートの特定を行います。
- **専門家 (Experts):** 約10GB × 100個（合計1TB）。BitNet b1.58 アーキテクチャと imatrix 圧縮により、10Bクラスの知識を10GBに収めたモデル。Hugging Faceのリポジトリに100個の .gguf ファイルとして配置。
